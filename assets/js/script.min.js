function init_navbar() {
  var a = $("#masthead"),
    b = $(a).outerHeight();
  $(".site-header-affix-wrapper").css("height", b);
  var c;
  c = $(".topbar").outerHeight();
  var d = $("body").hasClass("home");
  ($(window).scroll(function () {
    $(window).scrollTop() > c
      ? ($(a).addClass("sticky-bar"), d && $(a).addClass("header-dark"))
      : ($(a).removeClass("sticky-bar"), d && $(a).removeClass("header-dark"));
  }),
    $(window).scrollTop() > c &&
      ($(a).addClass("sticky-bar"), d && $(a).addClass("header-dark")));
}
function init_lightbox() {
  $(".image-lightbox").magnificPopup({
    type: "image",
    gallery: { enabled: !0 },
    mainClass: "mfp-with-zoom mfp-img-mobile",
    zoom: {
      enabled: !0,
      duration: 100,
      easing: "ease-in-out",
      opener: function (a) {
        return a.is("img") ? a : a.find("img");
      },
    },
  });
}

function init_workfilter() {
  var b = $(".work-grid");
  (b.imagesLoaded(function () {
    b.isotope({
      filter: "*",
      animationOptions: { duration: 750, easing: "easeOutBounce", queue: !1 },
    });
  }),
    $(".work-filter li").on("click", function () {
      ($(".work-filter li.active").removeClass("active"),
        $(this).addClass("active"));
      var a = $(this).attr("data-filter");
      return (
        b.imagesLoaded(function () {
          b.isotope({
            filter: a,
            animationOptions: {
              duration: 750,
              easing: "easeOutBounce",
              queue: !1,
            },
          });
        }),
        !1
      );
    }));
}

function init_formfocus() {
  $(".contact-form .form-control").on("focus", function () {
    ($('label[for="' + $(this).attr("id") + '"]').addClass("label-focus"),
      $(this).on("keyup", function () {
        var a = $(this).val().length;
        0 === a
          ? $('label[for="' + $(this).attr("id") + '"]').removeClass(
              "label-focus",
            )
          : $('label[for="' + $(this).attr("id") + '"]').addClass(
              "label-focus",
            );
      }),
      $(this).on("focusout", function () {
        0 === $(this).val().length
          ? $('label[for="' + $(this).attr("id") + '"]').removeClass(
              "label-focus",
            )
          : $('label[for="' + $(this).attr("id") + '"]').addClass(
              "label-focus",
            );
      }));
  });
}
function init_clientcarousel() {
  var a = $(".slider-carousel");
  a.imagesLoaded(function () {
    a.owlCarousel({
      autoPlay: 1500,
      slideSpeed: 400,
      stopOnHover: !0,
      items: 5,
      itemsDesktop: [1199, 5],
      itemsDesktopSmall: [979, 4],
      itemsTablet: [600, 3],
      itemsMobile: [479, 2],
      navigation: !0,
      navigationText: [
        '<i class="fa fa-angle-left"></i>',
        '<i class="fa fa-angle-right"></i>',
      ],
    });
  });
}
function init_bscarousel() {
  $(".carousel").carousel({ interval: 6500, pause: "hover" });
}
function init_popupframe() {
  $(".popup-video, .popup-gmaps").magnificPopup({
    disableOn: 700,
    type: "iframe",
    mainClass: "mfp-fade",
    removalDelay: 160,
    preloader: !1,
    fixedContentPos: !1,
  });
}
jQuery(document).ready(function (a) {
  "use strict";
  (jQuery(window).trigger("resize").trigger("scroll"),
    init_navbar(),
    init_lightbox(),
    init_workfilter(),
    init_formfocus(),
    init_clientcarousel(),
    init_bscarousel(),
    init_popupframe());
});
